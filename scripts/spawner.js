...
export class LootSpawner {
    ...
    static async spawnInRegion(regionDoc, config) {
        ...
        if (tokensToCreateData.length > 0) {
            ...
            // 3. Превращаем их в THM контейнеры и наполняем УНИКАЛЬНЫМ лутом
            for (const tokenDoc of createdTokens) {
                const token = tokenDoc.object; // Объект на сцене
                
                // Делаем лутабельным (если установлен модуль THM)
                ...
                if (thmModule?.active) {
                    ...
                    // Получаем доступ к API THM
                    const thmModule = game.modules.get("treasure-hoard-manager");
                    console.log(`SLS DEBUG | THM модуль:`, thmModule);
                    console.log(`SLS DEBUG | THM API:`, thmModule?.api);
                    console.log(`SLS DEBUG | THM Manager:`, thmModule?.manager);
                    
                    if (thmModule?.api?.hoardManager) {
                        console.log(`SLS DEBUG | THM HoardManager найден, создаем контейнер...`);
                        
                        // Создаем контейнер через THM
                        await thmModule.api.hoardManager.createHoard(token.actor, {
                            enabled: true,
                        ...
                        }, token); // Передаем токен для обновления внешнего вида
                        ...
                        // Генерируем лут для этого токена
                        ...
                        if (uniqueLoot.length > 0) {
                            ...
                            try {
                                ...
                                console.log(`SLS DEBUG | Предметы добавлены в контейнер THM:`, token.name);
                        ...
                        if (currency && (currency.cp || currency.sp || currency.gp)) {
                            ...
                            console.log(`SLS DEBUG | Валюта добавлена в контейнер THM:`, newCurrency);
                        } else {
